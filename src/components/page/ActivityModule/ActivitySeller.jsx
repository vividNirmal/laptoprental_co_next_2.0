...
const ActivitySeller = () => {
  ...
  const fetchActivities = async () => {
    setLoading(true);
    try {
      const url = `/user-activity-list?user_type=User&search=${encodeURIComponent(
        searchTerm
  ...
  return (
    <>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2">
        <div className="text-xs text-muted-foreground">Total Entries : {totalActivities}</div>
      ...
      {totalActivities > 0 && (
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="text-xs text-muted-foreground">
            Showing {(currentPage - 1) * selectedLimit + 1} to {Math.min(currentPage * selectedLimit, totalActivities)} of {totalActivities} entries
          </div>
