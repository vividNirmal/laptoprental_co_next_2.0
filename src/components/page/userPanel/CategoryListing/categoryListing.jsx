...
export default function CategoryListing({ metadata }) {
  ...
  useEffect(() => {
    // Only call API if categories are not already loaded
    if (!categories || categories.length === 0) {
      ...
      // Categories already loaded, just turn off loader
      dispatch(handlePageloader(false));
    }
  }, [categories, fetchHomepage, dispatch]);
  ...
  const Categorylayout = useMemo(() => {
    ...
    switch (layoutStyle) {
      case "1":
        return (
          <ul
            className={`grid grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-1.5 gap-x-[3px] max-[360px]:gap-1 `}
          >
            {categories?.length > 0 &&
            ...
            {categories?.length > 0 &&
              categories?.map((category, index) => (
                ...
                >
                  ...
                  <h2 className="p-2 md:p-3 text-xs relative [@media(max-width:360px)]:text-[10px] sm:text-xs 2xl:text-base text-center font-semibold text-black whitespace-nowrap max-w-full text-ellipsis overflow-hidden">
                    {category?.name}
                  </h2>
                </li>
              ))}
  ...
  }, [categories, staticdata?.home_page_layout_style]);
  ...
  return (
    <section className="relative grow">
        ...
        {/* <div className="pb-3">
          <div
            dangerouslySetInnerHTML={{
              __html: staticdata?.desktop_description,
            }}
      ...
      <div className="container mx-auto px-2.5">
        {/* <div className="my-4 w-full overflow-hidden md:rounded-xl" /> */}
      </div>
    </section>
  );
}
