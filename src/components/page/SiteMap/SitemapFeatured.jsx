...
const SitemapFeatured = () => {
  ...
  const fetchSitemaps = async () => {
    ...
    try {
      const res = await apiGet(
        "/generated-sitemap-urls?module_name=featured&type=three"
      );
      if (res.status === 1) {
  ...
  const handleGenerate = async () => {
    setLoading(true);
    try {
      const res = await apiGet("/generate-featured-listing-sitemap");
      if (res.status === 1) {
        // fetchSitemaps();
        setUrls(res.data || []);
...
export default SitemapFeatured;
