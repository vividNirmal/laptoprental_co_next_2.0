...
const SitemapJobListing = () => {
  ...
  const fetchSitemaps = async () => {
    ...
    try {
      const res = await apiGet(
        "/generated-sitemap-urls?module_name=job_listing&type=null"
      );
      if (res.status === 1) {
  ...
  const handleGenerate = async () => {
    setLoading(true);
    try {
      const res = await apiGet("/generate-listing-sitemap");
      if (res.status === 1) {
        setUrls(res.data || []);
...
export default SitemapJobListing
