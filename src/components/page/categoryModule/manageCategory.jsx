...
export default function ManageCategorypage() {
  ...
  const fetchCategories = async () => {
    ...
    try {
      const response = await getRequest(
        `get-category-list?search=${searchTerm}&page=${currentPage}&limit=${pageSize}`
      );
  ...
  const confirmDeleteAll = async () => {
    ...
    try {
      const res = await getRequest(`delete-all-category`);
      if (res.status == 1) {
    ...
    <>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2">
        ...
        <div className="flex flex-wrap gap-2">
          <Link href="/dashboard/manage-category/add-category">
            <Button>
          ...
          <ExportButton endpoint="export-category" fileName="Categories.csv" />
            ...
            <TableHeader>
              <TableRow>
                <TableHead className="w-[50px]">
                  <Checkbox
                    checked={
                      selectedCategory.size === categories.length &&
                      categories.length > 0
                    }
                ...
                categories.map((item, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell className="flex items-center justify-center gap-2">
                      <Link
                        href={`/dashboard/manage-category/add-category/${item._id}`}
                      >
                        <Button size="sm">
