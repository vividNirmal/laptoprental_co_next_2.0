...
function AddListinreview() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        let response;
        response = await postRequest("add-listing-review", formdata);
        if (response.data) {
          router.push("/dashboard/listing-review");
      ...
      <div className="flex flex-wrap gap-4 2xl:gap-5">
        <div className="space-y-2  w-full flex-grow">
          ...
          <CustomCombobox
            ...
            onChange={(value) => formik.setFieldValue("listing_id", value)}
          ...
          {formik.touched.listing_id && formik.errors.listing_id && (
            <p className="text-xs text-red-500">{formik.errors.listing_id}</p>
          )}
        </div>
        <div className="space-y-2  w-full flex-grow">
          <Label
...
export default AddListinreview;
