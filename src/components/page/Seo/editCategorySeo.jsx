...
export default function EditCategorySEOForm({ id }) {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        const response = await postRequest("update-category-seo", formData);
        if (response?.data) {
          router.push("/dashboard/category-seo");
    ...
    const fetchData = async () => {
      try {
        const res = await getRequest(
          `get-category-seo-details?category_seo_type=${category_seo_type}&category_id=${id}`
        );
        const seo = res?.data;
        if (seo) {
          setInitialValues({
