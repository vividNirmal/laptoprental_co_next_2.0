...
export default function ManageCategorySeo() {
  ...
  const fetchCategorySeo = async () => {
    ...
    try {
      const response = await getRequest(
        `category-seo-list?search=${searchTerm}&page=${currentPage}&limit=${pageSize}`
      );
  ...
  return (
    <>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2">
        ...
        <ExportButton
          endpoint="export-category-seo-details?type=1"
          fileName="Seo categories.csv"
                ...
                seo.map((s) => (
                  <TableRow key={s.categoryId} className="text-gray-500">
                    ...
                    <TableCell className="flex items-center justify-center gap-2">
                      <Link href={`/dashboard/category-seo/edit-category/${s.categoryId}`}>
                        <Button size="sm">
