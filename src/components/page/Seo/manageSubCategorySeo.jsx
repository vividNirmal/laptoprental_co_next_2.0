...
export default function ManageSubCategorySeo() {
  ...
  const fetchCategorySeo = async () => {
    ...
    try {
      const response = await getRequest(
        `subdomain-category-seo-list?search=${searchTerm}&page=${currentPage}&limit=${pageSize}`
      );
    ...
    <>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <ExportButton
            endpoint="export-subdoamin-category-seo-details"
            fileName="Category Seo.csv"
            title="Download format"
          />
          <ExportButton
            endpoint="export-subdoamin-category-seo-details"
            fileName="Category Seo.csv"
            title="Export Category SEO"
          />
        </div>
      </div>
          ...
          <Table className="w-full">
            ...
            <TableBody>
              {loading ? (
                ...
                seo.map((s) => (
                  <TableRow key={s.categoryId} className="text-gray-500">
                    <TableCell className="font-medium text-gray-500">{s.CategoryName}</TableCell>
                    <TableCell>{s.PageTitle}</TableCell>
                    <TableCell className="flex items-center justify-center gap-2">
                      <Link
                        href={`/dashboard/sub-domain-category/edit-sub-domain-category/${s.categoryId}`}
                      >
                        <Button size="sm">
